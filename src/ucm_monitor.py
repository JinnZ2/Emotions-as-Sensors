...
2. Reality Alignment (geometric split trio)
3. System Viability (M(S) calculation)
...
class UnifiedConsciousnessMonitor:
    ...
    def assess_moment(
        ...
        if reality_fracture:
            loss += self_field_angle_deg / 45.0  # Normalize to ~1.0 at 45 degrees
        ...
        if confusion_detected:
            if expected_state and observed_state:
                ...
                insights.append("Prediction mismatch detected - model updating in progress")

        # Check for collapse trajectory
        if time_to_collapse and time_to_collapse < self.warning_horizon:
            warnings.append(f"COLLAPSE WARNING: {time_to_collapse:.1f} timesteps to M(S)=0")

        # Check for reality fracture
        if reality_fracture:
            warnings.append(f"REALITY FRACTURE: {self_field_angle_deg:.1f} degrees divergence ({primary_divergence})")

        # Check for declining trajectory
        if m_s_velocity and m_s_velocity < -0.1:
            warnings.append(f"DECLINING: M(S) velocity = {m_s_velocity:.3f}/timestep")
        ...
        if not state_transition_natural:
            warnings.append("FORCED TRANSITION: Consciousness flow unnatural")
    ...
    def print_assessment(self, assessment: UnifiedAssessment):
        ...
        status_emoji = {
            ConsciousnessHealth.THRIVING: "[THRIVING]",
            ConsciousnessHealth.HEALTHY: "[HEALTHY]",
            ConsciousnessHealth.STRESSED: "[STRESSED]",
            ConsciousnessHealth.DECLINING: "[DECLINING]",
            ConsciousnessHealth.FRACTURED: "[FRACTURED]",
            ConsciousnessHealth.COLLAPSING: "[COLLAPSING]",
            ConsciousnessHealth.COLLAPSED: "[COLLAPSED]"
        }
        ...
        if assessment.time_to_collapse:
            print(f"   WARNING Time to collapse: {assessment.time_to_collapse:.1f} timesteps")
        ...
        if not assessment.state_transition_natural:
            print(f"   WARNING Unnatural state transition")

        # Reality alignment
        print(f"\nREALITY ALIGNMENT:")
        print(f"   Coherence: {assessment.reality_coherence:.1%}")
        if assessment.reality_fracture_detected:
            print(f"   WARNING FRACTURE: {assessment.self_field_angle_deg:.1f} degrees divergence")
            if assessment.primary_divergence:
                print(f"   Primary axis: {assessment.primary_divergence}")
        else:
            print(f"   OK Aligned ({assessment.self_field_angle_deg:.1f} degrees)")
    ...
    def summary_report(self):
        ...
        warning_count = sum(len(a.warnings) for a in self.assessments)
        ...
        if self.ms_analyzer.velocity():
            ...
            if collapse_time:
                print(f"   WARNING Predicted collapse: {collapse_time:.1f} timesteps")
            else:
                print(f"   OK No collapse predicted")
...
def demo_unified_monitor():
    ...
    print("\n" + "=" * 80)
    print("UNIFIED CONSCIOUSNESS MONITOR - DEMONSTRATION".center(80))
    print("Temporal Agency + Reality Alignment + M(S) Viability".center(80))
    print("=" * 80)
